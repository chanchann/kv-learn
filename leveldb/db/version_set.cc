...
// 1. 在对 Level-0 层的文件进行查找时，系统会将每一个有可能包含这个 key
// 的文件都加入到待搜查列表中，
// 随后将选中的文件从新到旧进行排序（新文件中存储的数据才是有效的数据），再对按序对每个文件进行查找。
// 2. 在对
// Level-N（N>0）层的文件进行查找时，系统在每一层中通过二分查找找到了一个候选文件。如果这个文件不包含当前的
// key，那么这一层的查找就结束了
// 3. 之所以对 Level-0 层的查找需要选取许多候选文件进行多次搜索，而对 Level-N（N
// > 0）层的文件查找只需一次 是因为 Level-0 层的文件都是从数据库中直接 dump
// 出来的，有可能存在前文所描述的数据重复的情况。 而从 Level-1
// 层开始，数据文件都是从上一层文件中合并得到的。
// 在合并的过程中，对同一条数据的操作只会保留一条，并且所有的文件会按照 key
// 的顺序排列，整个 Level-1 层及以上的文件是有序的。
void Version::ForEachOverlapping(Slice user_key, Slice internal_key, void* arg,
  ...
  if (!tmp.empty()) {
    ...
    for (uint32_t i = 0; i < tmp.size(); i++) {
      // 这里调用State::Match函数，每次都会把磁盘上的SSTable打开，加载到table_cache，之后查找
      if (!(*func)(arg, 0, tmp[i])) {
        return;
      }
    }
...
// 2. 大于 level
// 0的各层文件间是有序的，如果放到对应的层数会导致文件间不严格有序，会影响读取，则不再尝试。
// 3. 如果放到 level + 1层，与 level + 2层的文件重叠很大，就会导致 compact
// 到该文件时，压力过大，则不再尝试。这算是一个预测，放到 level
// 层能够缓冲这一点。
...
// 辅助类，实现Version + VersionEdit = Version
class VersionSet::Builder {
 ...
 public:
  ...
  // SaveTo就是将levels_记录的 deleted/added files 作用于原来的，生成新的version
  // v，就是遍历每一层，将 delta 的文件更新到合适位置。
...
// 在currentversion上应用指定的VersionEdit，生成新的MANIFEST信息，保存到磁盘上，并用作current
// version
Status VersionSet::LogAndApply(VersionEdit* edit, port::Mutex* mu) {
  ...
  {
    // Builder 对象将 VersionEdit 中的所有更改应用于当前版本，并将新版本保存到
    // Version 对象中 构建当前的版本version，委托给建造器进行构建
    Builder builder(this, current_);
    builder.Apply(edit);
    builder.SaveTo(v);
  }
  // 关键方法：内部通过打分机制确定文件所在的层级
  Finalize(v);

  // Initialize new descriptor log file if necessary by creating
  // a temporary file that contains a snapshot of the current version.
  // 如果MANIFEST文件指针不存在，就创建并初始化一个新的MANIFEST文件。
  // 这只会发生在第一次打开数据库时。这个MANIFEST文件保存了current
  // version的快照。
  ...
  // version的信息。在文件写操作时unlock锁，写入完成后，再重新lock，以防止浪费在长时间的IO操作上。
